...
const Dialogs = () => {
    ...
    const renderDescription = useMemo(() => {
        if (isOpened && componentProps.dialog?.description) {
            return <div className="text-[15px] text-slate-50 my-[30px]">{t(componentProps.dialog.description)}</div>;
        }
    ...
    const renderTitle = useMemo(() => {
        ...
        return  <div className={'flex items-center space-x-2'}>
            <div className={`flex items-center justify-center w-[40px] h-[40px] ${icon.backgroundColor} rounded-full`}>
