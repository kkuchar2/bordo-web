...
import {FormError} from "components/Forms/FormErrors/FormError";
...
export const FormErrors = (props: FormErrorsProps) => {
     ...
     * Get request errors
     */
    let requestErrors = getResponseError(errors, 'request');
     ...
     */
    let formErrors = getResponseError(errors, 'form', 'non_field_errors');

    /**
     * Filter generic and request errors by source - not displaying autologin errors
     */
    genericErrors = _.filter(genericErrors, error => error.source !== 'autoLogin');
    requestErrors = _.filter(genericErrors, error => error.source !== 'autoLogin');

    /**
     * Merge generic application errors with non-field form errors
     */
    const targetErrors = genericErrors.concat(formErrors).concat(requestErrors);
